...
public class TitleValidator implements Validator {
	...
	private static final String TITLE_PATTERN = "^[_A-ZÁ-ÿa-z0-9-]{1,45}+";
	...
	public void validate(FacesContext context, UIComponent component, Object submittedValue) {
		...
		if (oldValue != null && newTitle != null) {
			...
			if (oldTitle.getTitleId() != newTitle.getTitleId()) {
				throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_WARN, "Ce titre existe déjà", ""));
			}
		} else {
			// Mode ajout
			em.close();
			if (newTitle == null) {
				if (!matcher.matches()) {
					throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_WARN,
							"Mauvais format de titre. Seuls les alphanumériques sont acceptés. Longueur maximale 45 caractères.",
							""));
				}
			} else {
				throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_WARN, "Ce titre existe déjà.", ""));
